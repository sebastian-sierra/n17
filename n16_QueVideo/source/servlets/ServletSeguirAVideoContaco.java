...
public class ServletSeguirAVideoContaco extends HttpServlet
{
    ...
    private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
    {
        ...
        String emailUsuarioASeguir = request.getParameter("usuarioASeguir");
        
        Usuario usuario = SistemaGestionVideos.getInstance().darGrafoUsuarios().darVertice(new LlaveUsuario(emailUsuario)).darInfo();
        Usuario usuarioBuscado = SistemaGestionVideos.getInstance().darGrafoUsuarios().darVertice(new LlaveUsuario(emailUsuarioASeguir)).darInfo();

        SistemaGestionVideos.getInstance().adicionarVideoContadoAUsuario(emailUsuario, emailUsuarioASeguir);
        
        VistaVerUsuario vista = new VistaVerUsuario();
        vista.imprimirRespuesta(usuario, usuarioBuscado, response);
    }
