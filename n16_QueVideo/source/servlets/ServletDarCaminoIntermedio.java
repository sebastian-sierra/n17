...
import vistas.VistaVerCaminoIntermedio;
import vistas.VistaVerUsuario;
import vistas.VistaVerVideo;

public class ServletDarCaminoIntermedio extends HttpServlet
{
    ...
    {
        String emailUsuarioIntermedio = request.getParameter("usuario");
        String emailUsuarioOrigen = request.getParameter("idOrig");
        String emailUsuarioDestino = request.getParameter("idDest");
        
        Usuario usuarioInter = SistemaGestionVideos.getInstance().darGrafoUsuarios().darVertice(new LlaveUsuario(emailUsuarioIntermedio)).darInfo();
        Camino<LlaveUsuario, Usuario, Amistad> camino = SistemaGestionVideos.getInstance().darCaminoConUsuarioIntermedio(new LlaveUsuario(emailUsuarioOrigen), new LlaveUsuario(emailUsuarioDestino), new LlaveUsuario(emailUsuarioIntermedio));
        //TODO Crear Vista
        VistaVerCaminoIntermedio vista = new VistaVerCaminoIntermedio();
        vista.imprimirRespuesta(usuarioInter, emailUsuarioOrigen, emailUsuarioDestino, camino, response);
