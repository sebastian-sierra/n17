...
import vistas.VistaVerAdministrador;
import vistas.VistaVerUsuario;
import vistas.VistarErrorInicio;

public class ServletIniciarSesionDeUsuario extends HttpServlet
{
	...
	private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
	{
		...
		String contraseniaUs = request.getParameter("contrasenia"); 
		System.out.println(emailUsuario+" "+contraseniaUs);


		Usuario usuarioResultado = SistemaGestionVideos.getInstance().validarUsuario(emailUsuario, contraseniaUs);

		if(SistemaGestionVideos.getInstance().esAdministrador(emailUsuario, contraseniaUs))
		{
			VistaVerAdministrador vista = new VistaVerAdministrador();
			vista.imprimirRespuesta(SistemaGestionVideos.getInstance().darAdministrador(), response);
		}

		else if(usuarioResultado!=null)
		{
			System.out.println("Encontro al usuario");
			VistaVerUsuario vista = new VistaVerUsuario();
			vista.imprimirRespuesta(usuarioResultado, response);
		}
		...
		else
		{
			...
			vista.imprimirRespuesta("Aseg&uacute;rese de que su nombre de usuario y contrase&ntilde;a son correctos",response);
		}
