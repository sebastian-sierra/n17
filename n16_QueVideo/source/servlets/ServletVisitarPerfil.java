...
public class ServletVisitarPerfil extends HttpServlet
{
	...
	private void procesarSolicitud( HttpServletRequest request, HttpServletResponse response ) throws IOException
	{
		...
		String emailUsuarioActual = request.getParameter("usuario");
		String emailUsuarioAVisitar = request.getParameter("usuarioAVisitar");
		
		Usuario usuarioActual = SistemaGestionVideos.getInstance().darGrafoUsuarios().darVertice(new LlaveUsuario(emailUsuarioActual)).darInfo();
		Usuario usuarioAVisitar = SistemaGestionVideos.getInstance().darGrafoUsuarios().darVertice(new LlaveUsuario(emailUsuarioAVisitar)).darInfo();
		
		VistaVerUsuario vista = new VistaVerUsuario();
		vista.imprimirRespuesta(usuarioActual, usuarioAVisitar, response);
	}
